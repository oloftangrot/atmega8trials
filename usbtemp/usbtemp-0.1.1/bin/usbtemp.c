...
#include <linux/limits.h> /* PATH_MAX is used in usb.h but was not defined in my limits.h. */ 
...
static usb_dev_handle      *handle = NULL; // The local pointer to USB-control structures
#define min(a, b) (((a) < (b)) ? (a) : (b)) // Not safe to use if a or b is a value that can change between subsequent 
#define MAX_NUMBER_OF_SENSORS (64)
char foundSensors [MAX_NUMBER_OF_SENSORS * 8] = { 0 };  // Storage that contains ID-strings after search.
int numberOfSensors;

int init( char const * const device_id );
void usbTest( int dev );
void usbClose( int dev );
char* findSensors( int dev ); 
char* findDevices( void );
int readSensor( int dev, char const * const id );
int openDevice( char *serial, int device_handle, unsigned char mask );
...
{
  unsigned char     buffer[8];
  unsigned char		  retval = SENSOR_QUERY_FAILURE; // !=0 if an error occured.
  int               nBytes;
...
int printSensors(usb_dev_handle *handle) {
	if ( NULL != findSensors( 0 ) ) { // The return buffer pointer is not stored 
		fprintf( stderr, "%d sensor(s) found\n", numberOfSensors );
		for ( int i = 0; i < numberOfSensors; i++ ) {
			...
			for ( int j = 1; j < 7; j++ ) {
				char id_part = foundSensors[ ( i * 8 ) + j];
				printhex_byte(id_part);
			}
			fprintf(stderr, " type: ");
			if ( foundSensors[ i * 8 ] == DS18S20_ID )
				fprintf(stderr, "(DS18S20)\n");
			else if (foundSensors[ i * 8 ] == DS18B20_ID)
...
int init( char const * const device_id )
{
	(void) device_id; // Device identifier is not used right now.
  ...
  if ( error ) {
	...
	return 0; // Right now only one possible device is handled.
}
/*
**
...
/*
** No found sensors -> numberOfSensors = -1, return value NULL
** Otherwise numberOfSens
*/
char* findSensors( int dev )
{
  unsigned char amount=0;
	...
	memset( foundSensors, 0, MAX_NUMBER_OF_SENSORS * 8 ); // Clear return data.
  if ( getNumSensors(handle, &amount ) ) {
	//	fprintf( stderr, "%d sensor(s) found, querying\n", amount );
		for ( int i = 0; i < min( amount, MAX_NUMBER_OF_SENSORS ); i++ ) { 
			nBytes = usb_control_msg(handle, 
				USB_TYPE_VENDOR | USB_RECIP_DEVICE | USB_ENDPOINT_IN, 
				USBTEMP_CMD_QUERY_SENSOR,	  // Command ID
				i,							  // Value 
				0,							  // Index 
				(char *) &foundSensors[8 * i], 
				8, 
				5000);
			if ( nBytes < 2 ) {
				if ( nBytes < 0 ) {
